...
## 3.1 Unit Tests
  ...
  he server must first be tested from the client by making three new accounts in the application with (username,password) such as : (Bob,Builder),(Barney,Dinosaur),(BeAutY, bEAsT). Then, on a bash shell, run the command "python ServerTest.py" to test the methods of the server on these accounts. To test the database, running "python TestDB.py" will test storaging of documents on the MongoDB database.
